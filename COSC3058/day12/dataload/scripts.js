...
// Add items from courses into ul element
document.addEventListener("DOMContentLoaded", function() {

    var ul = document.getElementById("courses");
    courses.forEach(course => {
        let li = document.createElement("li");  // produce an li element
        // fill out data for an li
        for(let key in course) {                
            let p = document.createElement("p");
            p.appendChild(document.createTextNode(course[key]));
            p.classList.add(key);
            li.appendChild(p);
        }
        ul.appendChild(li);
